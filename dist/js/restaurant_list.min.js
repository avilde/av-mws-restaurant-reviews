...
var _dbhelper = require('./utils/dbhelper.js');
...
fetchNeighborhoods = function fetchNeighborhoods() {
  _dbhelper.DBHelper.fetchNeighborhoods(function (error, neighborhoods) {
    if (error) {
      // Got an error
      console.error(error);
    } else {
...
fetchCuisines = function fetchCuisines() {
  _dbhelper.DBHelper.fetchCuisines(function (error, cuisines) {
    if (error) {
      // Got an error!
      console.error(error);
    } else {
...
updateRestaurants = function updateRestaurants() {
  ...
  _dbhelper.DBHelper.fetchRestaurantByCuisineAndNeighborhood(cuisine, neighborhood, function (error, restaurants) {
    if (error) {
      // Got an error!
      console.error(error);
    } else {
...
addMarkersToMap = function addMarkersToMap() {
  ...
  restaurants.forEach(function (restaurant) {
    ...
    var marker = _dbhelper.DBHelper.mapMarkerForRestaurant(restaurant, self.map);
